...
public class PistaPremiumStrategy implements TipoIngressoStrategy {
    ...
    public BigDecimal calcularValorIngresso(Usuario usuario) {
        ...
        if (usuario.isEstudante() && usuario.getDataNascimento().getYear() - LocalDate.now().getYear() < 60) {
            valorTotalDoIngressoCom50DeDescontoParaEstudante = valorTotalDoIngressoPistaPremium
        ...
        if (!usuario.isEstudante() && usuario.getDataNascimento().getYear() - LocalDate.now().getYear() > 60) {
            valorTotalDoIngressoCom50DeDescontoParaIdoso = valorTotalDoIngressoPistaPremium
                    .subtract(cinquentaPorCentoDoTotal);
        ...
        if (usuario.isEstudante() == true && usuario.getDataNascimento().getYear() - LocalDate.now().getYear() > 60) {
            valorTotalDoIngressoCom50DeDescontoParaIdosoEstudante = valorTotalDoIngressoPistaPremium
                    .subtract(cinquentaPorCentoDoTotal);

            return valorTotalDoIngressoCom50DeDescontoParaIdosoEstudante;

        }
        return valorTotalDoIngressoPistaPremium;
    }
